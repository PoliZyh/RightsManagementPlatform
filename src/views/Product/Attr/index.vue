      ...
      <div v-show="!showAttrList">
        ...
        >
          ...
          <el-table-column label="属性值名称">
            <template #="{ row, $index }">
              <el-input
                :ref="(vc: InputInstance) => (elInputArr[$index] = vc)"
                size="small"
                v-if="row.flag"
          ...
          <el-table-column label="属性值操作">
            <template #="{ $index }">
              <el-button
                ...
                size="small"
                icon="Delete"
                @click="handleDeleteAttrValue($index)"
              ></el-button>
            </template>
          </el-table-column>
        </el-table>
...
const handleToLook = (attrValueRow: I_ATTR_VALUE_OBJ, index: number) => {
  ...
  // 属性值不能重复
  const hasRepeat = attrParams.attrValueList.find(
    (item) =>
      item !== attrValueRow && item.valueName === attrValueRow.valueName,
  )
  if (hasRepeat) {
